...
import { View, StyleSheet, SafeAreaView, ScrollView, Platform, Alert } from 'react-native';
...
export default function OnboardingScreen({ route, navigation }: any) {
  ...
  React.useEffect(() => {
    const checkFirstTime = () => {
      try {
        const hasShown = storage.getString('hasShownUsernameSheet');
        if (hasShown !== 'true') {
  ...
  const [githubStatus, setGithubStatus] = useState<PlatformStatus>('unconnected');
  ...
  const [codechefStatus, setCodechefStatus] = useState<PlatformStatus>('unconnected');
  ...
  const { data: linkedPlatformsData } = useGetLinkedPlatformsQuery();
  const [modalError, setModalError] = useState<string | null>(null);

  const getPlatformStatus = (enumValue: string, localStatus: PlatformStatus): PlatformStatus => {
    if (localStatus === 'connecting') return 'connecting';
    const isLinked = linkedPlatformsData?.platformLinks?.some(p => p.platform === enumValue);
    if (isLinked) return 'connected';
    return localStatus;
  };

  const finalGithubStatus = getPlatformStatus('GITHUB', githubStatus);
  const finalLeetcodeStatus = getPlatformStatus('LEETCODE', leetcodeStatus);
  const finalCodeforcesStatus = getPlatformStatus('CODEFORCES', codeforcesStatus);
  const finalCodechefStatus = getPlatformStatus('CODECHEF', codechefStatus);
  const finalGfgStatus = getPlatformStatus('GEEKSFORGEEKS', gfgStatus);

  const connectedCount = [
    finalGithubStatus,
    finalLeetcodeStatus,
    finalCodeforcesStatus,
    finalCodechefStatus,
    finalGfgStatus
  ].filter(s => s === 'connected').length;

  const handleConnectPlatform = async (id: string, input: string) => {
    let platformEnum = '';
    let setStatusFn;
    ...
    if (id === 'github') {
      setStatusFn = setGithubStatus;
      platformEnum = 'GITHUB';
    } else if (id === 'leetcode') {
      setStatusFn = setLeetcodeStatus;
      platformEnum = 'LEETCODE';
    } else if (id === 'codeforces') {
      setStatusFn = setCodeforcesStatus;
      platformEnum = 'CODEFORCES';
    } else if (id === 'codechef') {
      setStatusFn = setCodechefStatus;
      platformEnum = 'CODECHEF';
    } else if (id === 'geeksforgeeks') {
      setStatusFn = setGfgStatus;
      platformEnum = 'GEEKSFORGEEKS';
    }

    if (!setStatusFn) return;

    setStatusFn('connecting');
    ...
    <SafeAreaView style={styles.container}>
        ...
        <View style={styles.contentWrapper}>
          ...
          <View style={styles.headerSection}>
            <View style={styles.titleRow}>
              ...
              <View style={styles.badge}>
                <AppText style={styles.badgeText}>{connectedCount}/5 connected</AppText>
              </View>
            </View>
          ...
          >
            <View style={styles.platformsSection}>
              <PlatformCard
                name="GitHub"
                description={finalGithubStatus === 'connected' ? `Connected as @${username}` : "Code hosting & collaboration"}
                brandColor="#ffffff"
                ...
                status={finalGithubStatus}
                onConnect={() => setActivePlatform({ id: 'github', name: 'GitHub', brandColor: '#ffffff', placeholder: 'Enter GitHub username' })}
              />
              ...
              <PlatformCard
                ...
                status={finalLeetcodeStatus}
                onConnect={() => setActivePlatform({ id: 'leetcode', name: 'LeetCode', brandColor: '#FFA116', placeholder: 'Enter LeetCode username' })}
              />
              ...
              <PlatformCard
                ...
                status={finalCodeforcesStatus}
                onConnect={() => setActivePlatform({ id: 'codeforces', name: 'Codeforces', brandColor: '#3182ce', placeholder: 'Enter Codeforces username' })}
              />
      ...
      {activePlatform && (
        <LinkPlatformModal
          ...
          onConnect={(url) => {
            handleConnectPlatform(activePlatform.id, url);
          }}
        />
      )}
      ...
      <UsernameConfirmationSheet
        ...
        onClose={() => {
          ...
          storage.set('hasShownUsernameSheet', 'true');
        }}
