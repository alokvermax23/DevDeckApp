...
export default function OnboardingScreen({ route, navigation }: any) {
  ...
  React.useEffect(() => {
    const checkFirstTime = async () => {
      try {
        const hasShown = await AsyncStorage.getItem('hasShownUsernameSheet');
        if (hasShown !== 'true') {
    ...
    <SafeAreaView style={styles.container}>
      ...
      <UsernameConfirmationSheet 
        ...
        onClose={() => {
          ...
          AsyncStorage.setItem('hasShownUsernameSheet', 'true').catch(console.error);
        }} 
